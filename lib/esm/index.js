...
(function (State) {
    ...
    State.use = (state) => {
        const [_state, _setState] = React.useState({ internal: (isState(state) ? state[INTERNAL] : State.create(state)[INTERNAL]) });
        React.useEffect(() => {
    ...
    State.useObserve = (state, observer) => {
        return React.useEffect(() => State.observe(state, observer).remove, []);
    };
})(State || (State = {}));
